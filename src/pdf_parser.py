...
class PDFParser:
    """
    PDF parser for structured section extraction with heading detection,
    title parsing, hierarchy analysis, JSON export, and full parsing log.
    """
    ...
    def extract_sections_from_pdf(self, pdf_path: str) -> List[Dict]:
        ...
        try:
            ...
            if toc:
                ...
                sections = self._create_sections_from_toc(doc, toc, doc_name)
    ...
    def _process_with_heuristics(self, doc: fitz.Document, doc_name: str) -> List[Dict]:
        ...
        self._log_parsing_step("TEXT_BLOCKS_FOUND", f"Extracted {len(text_blocks)} text blocks", {
            ...
            "sample_blocks": [
                {"text": block.get("text", "")[:50] + "...", "size": block.get("size"), "page": block.get("page")}
        ...
        if toc_headings:
            self._log_parsing_step("VISUAL_TOC_FOUND", f"Found visual ToC with {len(toc_headings)} headings", {
                "toc_headings": [{"text": h.get("text"), "level": h.get("level"), "page": h.get("page")} for h in
                                 toc_headings]
            })
            self._extraction_method = "visual_toc"
            sections = self._create_sections_from_headings(doc, toc_headings, doc_name)
        else:
            ...
            self._log_parsing_step("HEADING_CANDIDATES",
                                   f"Found {len(heading_candidates)} potential heading candidates")

            headings = self._extract_headings_with_validation(
                heading_candidates, font_stats)
            ...
            sections = self._create_sections_from_headings(doc, headings, doc_name)
    ...
    def _create_output_structure(self, pdf_path: str, sections: List[Dict]) -> Dict:
        ...
        for i, section in enumerate(sections):
            ...
            self._log_parsing_step("SECTION_STRUCTURED",
                                   f"Structured section '{heading}' with {len(paragraphs)} paragraphs")
        ...
        self._log_parsing_step("OUTPUT_COMPLETE",
                               f"Final structure created with {len(processed_sections)} sections and {len(self.parsing_log)} log entries preserved")
    ...
    def _save_to_json(self, data: Dict) -> None:
        ...
        try:
            ...
            self._log_parsing_step("JSON_PREPARATION",
                                   f"Preparing to save data with complete parsing log to {self.output_file}")
    ...
    def _create_sections_from_headings(self, doc: fitz.Document, headings: List[Dict],
        ...
        for i, heading in enumerate(headings):
            try:
                ...
                self._log_parsing_step("SECTION_PROCESSING",
                                       f"Processing section {i + 1}: '{heading_text}' on page {start_page + 1}", {
                                           "section_number": i + 1,
                ...
                self._log_parsing_step("CONTENT_EXTRACTION",
                                       f"Extracting content from page {start_page + 1} to {end_page + 1}")
    ...
    def _extract_section_content(self, doc: fitz.Document, heading: Dict,
        ...
        """Extract content for a section with better handling of multipage sections."""
        content_parts = []
        ...
        for page_idx in range(start_page, min(end_page + 1, len(doc))):
            try:
                if page_idx == start_page:
                    ...
                    page_content = self._extract_content_before_next_heading(doc, page_idx)
                else:
    ...
    def _extract_content_before_next_heading(self, doc: fitz.Document, page_num: int) -> str:
    ...
    @staticmethod
    def _get_block_y_position(block: Dict) -> Optional[float]:
    ...
    @staticmethod
    def _is_page_element(line: str) -> bool:
    ...
    @staticmethod
    def _process_line(line: Dict) -> Optional[Dict]:
    ...
    @staticmethod
    def _lines_should_group(line1: Dict, line2: Dict) -> bool:
    ...
    @staticmethod
    def _clean_rfp_title(title: str) -> str:
    ...
    @staticmethod
    def _determine_toc_level(prefix: str) -> str:
            ...
            for page_num, blocks in page_blocks.items():
                ...
                for i, block in enumerate(blocks):
                    ...
                    if self._is_likely_heading(block, font_stats):
                        ...
                        if level and self._validate_heading_context(blocks, i):
    ...
    def _validate_heading_context(self, page_blocks: List[Dict], block_index: int) -> bool:
    ...
    @staticmethod
    def _determine_heading_level_smart(block: Dict, font_stats: Dict) -> Optional[str]:
    ...
    @staticmethod
    def _is_obvious_non_heading(text: str) -> bool:
...
def process_pdf_to_json(pdf_path: str, output_file: str = "checking.json") -> Dict:
    """
    Extracts sections from a PDF and saves results with full parsing log to JSON.

    Args:
        pdf_path (str): Path to the input PDF.
        output_file (str): Path to the output JSON file (default: "checking.json").

    Returns:
        Dict: Metadata and complete parsing log.
    """

    processor = PDFParser(output_file)
    return processor.process_pdf_and_save(pdf_path)


def extract_sections_from_pdf(pdf_path: str) -> List[Dict]:
    """
    Extracts structured sections from a PDF using visual and semantic cues.

    Features:
    - Detects headings via font size, style, and patterns
    - Supports multi-line titles (e.g., RFP-style)
    - Parses visual and embedded tables of contents
    - Removes headers/footers and validates section content
    - Includes error handling and a detailed processing log in JSON

    Args:
        pdf_path (str): Path to the PDF file

    Returns:
        List[Dict]: Extracted sections with:
            - section_title (str)
            - content (str)
